...
class CachedSession:
    token: str           # AdminToken from /user/login (URL-encoded form, verbatim)
    expires_at: int      # unix seconds; when we consider cache stale (server doesn't tell us)
...
class TokenStore(Protocol):
    ...
    def login_lock(
        ...
        """Serializes /user/login calls for a game across workers.
